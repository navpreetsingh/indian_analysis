class StrategyAnalysis < ActiveRecord::Base
	def self.analysis
		...
		dates = data.uniq.pluck(:date).sort

		dates.each do |date|		
			for k in 1..5
				for i in 1..6
					for j in 1..8
						d = data.where("bs_signal = 1 and date = ? and strategy = ?", date, (k*100) + (i*10) + j)
						total = d.count
						if total > 0
							...
							StrategyAnalysis.create(:strategy => (k*100) + (i*10) + j, 
								:date => date, :bs_signal => 1,
